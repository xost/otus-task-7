...
func createNotif(id int, message string) error {
	...
	req, err := http.NewRequest("POST", "http://notif.orders.svc.cluster.local:9000/notif/create", b)
	if err != nil {
		return err
	}
...
func getbalance(id int) (int, error) {
	req, err := http.NewRequest("GET", "http://account.orders.svc.cluster.local:9000/account/get", nil)
	if err != nil {
		return 0, err
	}
	req.Header.Set("X-User-Id", strconv.Itoa(id))
...
func deposit(id, amount int) error {
	...
	req, err := http.NewRequest("PUT", "http://account.orders.svc.cluster.local:9000/account/deposit", b)
	if err != nil {
		return err
	}
	req.Header.Set("X-User-Id", strconv.Itoa(id))
...
func create(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		log.Printf("Failed to get balance user id [%d]: %s\n", id, err)
		return
	}
	o := orderModel{}
	...
	if b-o.Amount < 0 {
		log.Printf("Insufficient funds in the account for user id [%d]\n", id)
		w.WriteHeader(http.StatusPaymentRequired)
		if err = createNotif(id, "Failed to create order due to insufficient funds in the account"); err != nil {
			log.Printf("Failed to create notification for user id [%d]: %s\n", id, err)
		}
		return
	}
	if err = deposit(id, 0-o.Amount); err != nil {
		log.Printf("Failed to withdrawal funds for user id [%d]: %s\n", id, err)
		w.WriteHeader(http.StatusPaymentRequired)
		if err = createNotif(id, "Failed to create order due to could not withdrawal funds from the account"); err != nil {
			log.Printf("Failed to create notification for user id [%d]: %s\n", id, err)
		}
		return
	}
	if err = createOrder(id, o.Amount, o.Item); err != nil {
		log.Printf("Failed to create order for user id [%d]: %s. Need to return funds on account.\n", id, err)
		w.WriteHeader(http.StatusInternalServerError)
		if err = createNotif(id, "Failed to create order. Your funds will be return on your account"); err != nil {
			log.Printf("Failed to create notification for user id [%d]: %s\n", id, err)
		}
		if err = deposit(id, o.Amount); err != nil {
			log.Printf("Failed to return funds for user id [%d]: %s\n", id, err)
		}
		return
	}
